...
let getLayoutHTML = require('./lib/getLayoutHTML');

app.get('/cookie-test', (request, response) => {
  // If we have no "count" cookie, request.cookies['count'] will be undefined
  // and parseInt(undefined) will be NaN
  let cookieCount = parseInt(request.cookies['count']);

  if (isNaN(cookieCount)) {
    cookieCount = 0;
  }

  // Add the "Set-Cookie: ..." header to the response with
  // the new value for the count cookie. Our browser will send
  // this new value in the next request it makes.
  response.cookie('count', cookieCount + 1);

  response.send(`The count is: ${cookieCount}`);
});
